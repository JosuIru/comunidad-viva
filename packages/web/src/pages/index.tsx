...
function HomePage() {
  ...
  // Transform data to map pins - wrapped in useMemo to prevent initialization errors
  const { offersWithCoords, eventsWithCoords, needsWithCoords, projectsWithCoords, housingWithCoords, timebankWithCoords, groupbuysWithCoords } = useMemo(() => ({
    offersWithCoords: (offersData || []).filter((offer: any) => offer.lat && offer.lng && offer.lat !== 0 && offer.lng !== 0),
    eventsWithCoords: (eventsData || []).filter((event: any) => event.lat && event.lng && event.lat !== 0 && event.lng !== 0),
    needsWithCoords: (needsData || []).filter((need: any) => need.latitude && need.longitude),
    projectsWithCoords: (projectsData || []).filter((project: any) => project.latitude && project.longitude),
    housingWithCoords: (housingData || []).filter((housing: any) => housing.latitude && housing.longitude),
    timebankWithCoords: (timebankData || []).filter((offer: any) => offer.offer?.lat && offer.offer?.lng && offer.offer?.lat !== 0 && offer.offer?.lng !== 0),
    groupbuysWithCoords: (groupbuysData || []).filter((gb: any) => gb.pickupLat && gb.pickupLng),
  }), [offersData, eventsData, needsData, projectsData, housingData, timebankData, groupbuysData]);
