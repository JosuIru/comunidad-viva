...
Se ha implementado exitosamente un **sistema completo de bridges blockchain** que permite a los usuarios de Truk transferir tokens SEMILLA entre la blockchain interna de la plataforma y blockchains externas públicas (Polygon y Solana).
...
**UNLOCK (Blockchain Externa → Truk)**:
```
...
Actualiza status a UNLOCKED
  ↓
Usuario recibe SEMILLA en Truk
```
...
- Volver a traer ganancias a Truk
